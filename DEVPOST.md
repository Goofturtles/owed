...
Version 1 of the rulebook holds 318 rules: 151 manufacturer warranties, 75 statutory rules, 47 card benefits, 15 settlements, 3 repair programmes and 27 shop policies. All 318 link to the page they came from. Every rule carries a confidence value, and 42 are marked something other than "certain" — 41 "likely", 1 "possible" — because I could only confirm them from a secondary source or with a caveat. The app shows that instead of hiding it.

Your shelf saves what you own. When the rulebook grows, everything on the shelf is re-checked and new matches are flagged as new. When a claim works you can mark it won from its script; that is the only outcome Owed records, and nothing is inferred from it.

## How this fits Earth Forward

Broken electronics are the fastest-growing household waste stream, and a lot of that gear dies over a fixable fault while somebody was contractually required to fix it for nothing. The cheapest repair in the world is the one somebody else already agreed to pay for. Owed's job is to make that repair the easy option instead of the obscure one. Regulators have already done the hard part: the EU now requires phone and tablet spare parts to stay on sale for seven years after a model stops being sold, plus at least five years of operating system updates. Rules like that only reduce waste if the person holding the broken thing knows they exist. That gap is where Owed sits. It is also why the only outcome Owed records is a claim you mark as won yourself. An invented CO2 figure would have looked better on the page and meant nothing.

## What it does not do

Saying this plainly is part of the design.

- The rulebook is hand-collected, so it is deep rather than complete. Unusual brands and models return nothing, and the app says "Nothing found for this one." rather than inventing a match.
...
No framework, no build step, no backend. Three pages: a landing page, a sign-in page, and the app. About 3,800 lines of plain JavaScript, and the only external request on the whole site is Google Fonts.
...
- The app is a three-pane shell. A sidebar holds your shelf ("My things"), a "Check something" button, the region picker, theme, help and sign out. The results column opens with one plain sentence ("11 places to ask about a free fix. Start with the first one below.") and a three-part key, then the real leads grouped by where the cover comes from, one card per rule: a source tile, a Strong / Worth asking / Long shot pill, the facts, and a "Get the words to say" button, with "Start here" on the top lead. Every long shot sits behind a single "Show 28 long shots" row at the end, so nothing is hidden and nothing breaks up the list. From 1200px a right-hand panel holds the selected rule and its script; under 900px the sidebar becomes a bottom tab bar (My things · Check · Help). The wizard asks one question per screen, four in all, each answered from a grid of large icon tiles, with "Can't remember" allowed for the date and the payment, and the Continue button names the next question. On the first question you can also take a photo of the item: it is shrunk and kept with the item in your browser, named on the device where the browser ships a built-in model (Chrome's Prompt API), read for a barcode otherwise, and never uploaded.
...
- Warm paper grounds with warm near-black ink and one green that reads as money coming back, plus a soft mint pill for anything you press. Geist carries the display headlines, the UI and the body; Geist Mono is the fine-print voice on the landing page; Instrument Serif appears in italic only, for the one emphasised clause in a headline. Light is the default for every visitor; dark is an explicit choice, and each theme ships its own product screenshot.
- The landing page opens into a scroll-driven WebGL flythrough of the paperwork: a corridor of pages of fine print the camera travels down, until the one clause that means a free repair lights up. It is raw WebGL in about 14KB — no Three.js — because a 600KB library would have outweighed the entire rest of the site.
- A black interlude sits mid-page: one headline hands over to the next word by word as you scroll, while the five places cover hides name themselves one at a time underneath. Scrolling down plays it through over about seven seconds — paced so the words can be read on the way past; scrolling up, any key or a pointer press releases it at any point, and it never grabs the same section twice. An ambient clip behind the closing call to action, generated on a local RTX 4090 with ComfyUI + LTX-Video rather than bought from an API, was cut when that section was rebuilt; `tools/generate_video.py` still reproduces it from the prompt committed in the repo.
- An initial visit is 11 requests and 208KB uncompressed (65KB gzipped). Everything heavy is deferred: the product screenshot (one per theme) and the four how-it-works crops load only when you scroll to them, so an initial load fetches none of it. The 3D scene is switched off entirely under `prefers-reduced-motion` — which also collapses the pinned scroll sections back to normal page flow instead of leaving empty scroll traps.
...
- All 318 rules link to their source. If Owed is wrong, you can catch it in one click.
- Blank fields stayed blank. 47 rules have no contact number or link, because I never saw one on a page I trusted and would not paste one from memory.
...
- Add a real "vehicle" category and re-tag the 14 car rules currently filed under "other". Car rules carry the biggest money in the book.
...
- Thicken the retailer layer. The shop-policy bucket holds 27 rules, 23 of them US.
