...
- A black interlude sits mid-page: one headline hands over to the next word by word as you scroll, while the five places cover hides name themselves one at a time underneath. Scrolling down plays it through; scrolling up releases it. The ambient clip behind the closing call to action was generated on a local RTX 4090 with ComfyUI + LTX-Video rather than bought from an API; `tools/generate_video.py` reproduces it from the prompt committed in the repo.
