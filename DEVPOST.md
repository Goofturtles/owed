...
- A second WebGL scene renders an isometric city the camera drifts across on scroll — every block is somewhere cover hides, and the five sources name themselves one at a time as you travel. The ambient clip behind the closing call to action was generated on a local RTX 4090 with ComfyUI + LTX-Video rather than bought from an API; `tools/generate_video.py` reproduces it from the prompt committed in the repo.
