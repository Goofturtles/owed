...
### The app and the sign-in (3 September 2026, night — one brand per screen)

The owner approved a horizontal storyboard of five Mobbin stills before any code (`storyboard/app/`, local only), then said "add some real images and make sure they fit in". Same rules as the white page: pure white, black type and buttons, Apple blue #0071E3 as the one accent (it replaces each reference's own accent slot), Geist, 16px minimum, 44px targets, less text. The stills were measured at 1440 (768px still × 1.875) and the specs kept the reference's geometry; the build changed only words, icons and Owed's content.

- **Sign-in — Emergent's login** ([5043c70e](https://mobbin.com/screens/5043c70e-4f46-457f-bce2-7c8bfe58ab11)): 45/55 split; white left cell with the faint character grid in two corners (a CSS data-URI tile, masked), the 364px centred column (mark, two-line heading with line two in Apple blue, one 47px pill field, a 49px black pill, a 49px grey pill, two lines of fine print); the right cell a 20px-inset radius-26 gradient panel with Emergent's exact stops, a translucent badge, a 28px white head, a 581×368 photo card with a 22px chrome bar (Emergent shows its product there; Owed shows a real photograph) and a dots pager with a pause control. Owed has no password, so one field.
- **One question per screen — Dribbble's onboarding step** ([16b97255](https://mobbin.com/screens/16b97255-e07f-4344-adf6-3eaea3b6e6a6)): a 424px column, a 45px accent icon, the bold question, one helper line, a 262×4 progress bar, a 3-up tile grid (2-up on phones), a black pill Next, Back/Cancel on a top rail; the four questions (what broke, who made it, how old, how paid) each get this screen; the first question's tiles carry 44px real-photo thumbnails.
- **Shelf + shell — X web's Home** ([236bea89](https://mobbin.com/screens/236bea89-df25-4585-9b43-f860fe1dbacd)): a 275px sidebar (mark, Start, Saved things with a count badge, the saved items, a black pill "Check something new", a region pill, icon tools), the centre column, a 350px rail (the ranking legend); hairlines #EFF3F4-style, no tinted panels.
- **Results — Cal.com's bookings list** ([114e5cd8](https://mobbin.com/screens/114e5cd8-b6c1-48a9-94b6-4ffd54b2095a)): one summary line, filter chips (All / Strong / Worth asking / Long shot), rows grouped by source under a grey uppercase band, each row a source word + status pill, a bold title with one short reason clause, and a black "Get script"; the row itself opens the document.
- **Rule + script — Mistral Le Chat's document card** ([e12a6005](https://mobbin.com/screens/e12a6005-acf0-47b7-b58d-f78694478847)): the document replaces the list in the main column — kicker + status pill, title, label/value rows (who to ask, rule, window with a "More" disclosure, deadline, contact, source), the script with the key phrases bold, a black Copy and an outlined Mark as won, "‹ Back to results" above.
- **Photographs** (all Pexels licence, free for commercial use, no attribution; stored as WebP at 1600 and 800 wide in `assets/img/photo/`): headphones 3394650, phone 6373160, laptop 34502068, washing machine 19846397, drill 30413424, cast-iron pan 1435909, coffee machine 6032797, running shoes 1456733, watch 20118030, sofa 12285892. Used only where a reference shows imagery: the sign-in panel, the first question's tiles and the shelf's empty state. Two were swapped after a first look (the phone and the watch read as dark blobs at 44px).
